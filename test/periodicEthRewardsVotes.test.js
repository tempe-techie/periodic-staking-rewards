...
describe("PeriodicEthRewardsVotes (with ERC20Votes and ERC20Permit)", function () {
  ...
  it("Scenario 1: User1 deposits 300 tokens, user2 deposits 700 tokens. They claim rewards after 1 week (and again a week later).", async function () {
    ...
    const txDeposit1 = await rewardsContract.connect(user1).deposit(user1tokensToDeposit);
    const receiptDeposit1 = await txDeposit1.wait();
    calculateGasCosts("deposit 1", receiptDeposit1);
  ...
  it("Scenario 7: asset token has 10 decimals (instead of 18). How does this affect the rewards calculation? How about withdrawals?", async function() {
    ...
    // user1 withdraws their staking tokens
    const txWithdraw1 = await rewardsContract3.connect(user1).withdraw(user1tokensToDeposit);
    const receiptWithdraw1 = await txWithdraw1.wait();
    calculateGasCosts("withdraw 1", receiptWithdraw1);
